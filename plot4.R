...
##open png device, create the frames, the four graphs and close device
png("plot4.png")
par(mfrow=(c(2,2)))
with(Data,plot(strptime(paste(Data$Date,Data$Time),"%d/%m/%Y %H:%M:%S"),Data$Global_active_power,type="l",ylab="Global Active Power",xlab=""))

with(Data,plot(strptime(paste(Data$Date,Data$Time),"%d/%m/%Y %H:%M:%S"),Data$Voltage,type="l",ylab="Voltage",xlab="daytime"))
...
with(Data,plot(strptime(paste(Data$Date,Data$Time),"%d/%m/%Y %H:%M:%S"),Data$Global_reactive_power,type="l",ylab="Global_reactive_power",xlab="daytime"))
